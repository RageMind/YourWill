(() => {
  ...
  const humans = [
    makeHuman('human_alina', 'Алина', -2.5, 0.5, '#c77b43'),
    makeHuman('human_mir', 'Мир', 2.5, -0.5, '#6c8f45')
  ];
  ...
  function floorDiv(v, size) {
    return Math.floor(v / size);
  }

  function localIndex(tx, ty) {
    return ((ty % CHUNK + CHUNK) % CHUNK) * CHUNK + ((tx % CHUNK + CHUNK) % CHUNK);
  }

  function chunkOfTile(tx, ty) {
    return { cx: floorDiv(tx, CHUNK), cy: floorDiv(ty, CHUNK) };
  }
  ...
  function makeHuman(id, name, x, y, color) {
    return {
      ...
      interactions: ['walk', 'explore', 'pick_flower', 'gather_apple', 'eat_apple']
    };
  }

  function ensureChunk(cx, cy, reason = 'unknown') {
    const k = key(cx, cy);
    ...
    for (let ly = 0; ly < CHUNK; ly += 1) {
      for (let lx = 0; lx < CHUNK; lx += 1) {
        ...
        const objects = makeObjectsOnTile(tile, cx, cy);
        for (const object of objects) chunk.objects.push(object);
      }
    }
  ...
  function makeTile(tx, ty, cx, cy) {
    const island = 1 - Math.hypot(tx / 95, ty / 78);
    const elev = island + (fbm(tx * 0.035, ty * 0.035, SEED, 5) - 0.5) * 0.48;
    const wet = fbm(tx * 0.07 + 50, ty * 0.07 - 20, SEED + 70, 4);
    const fertility = Math.max(0, Math.min(1, wet * 0.7 + (1 - Math.abs(elev - 0.42)) * 0.4));
    ...
    if (elev < 0.14) {
      ...
      biome = 'meadow';
      height = 0.65;
    } else if (fbm(tx * 0.05 - 60, ty * 0.05 + 30, SEED + 200, 4) > 0.63) {
      type = 'forest_floor';
      biome = 'forest';
      height = 0.75;
    }
    ...
    return {
      ...
      height: Math.round((height + elev * 0.55) * 100) / 100,
      biome,
      moisture: Math.round(wet * 100) / 100,
      fertility: Math.round(fertility * 100) / 100,
      passable,
      walkCost: type === 'forest_floor' ? 1.7 : type === 'stone' ? 1.4 : 1,
      resources: {
  ...
  function makeObjectsOnTile(tile, cx, cy) {
    const r = hash(tile.x, tile.y, SEED + 900);
    const out = [];
    if (tile.type === 'water' || tile.type === 'sand') return out;

    if (tile.type === 'forest_floor' && r > 0.78) {
      out.push(makeObject('apple_tree', tile, 1, 1, {
        hp: 80,
        height: 2.2,
        growth: 70 + Math.floor(hash(tile.x, tile.y, SEED + 1) * 30),
        resources: { wood: 6, apple: 0 },
        interactions: ['inspect', 'gather_apple', 'chop_tree']
      }));
    } else if (tile.type === 'flower_grass' && r > 0.74) {
      out.push(makeObject('flower', tile, 1, 1, {
        hp: 8,
        height: 0.25,
        growth: Math.floor(hash(tile.x, tile.y, SEED + 2) * 100),
        resources: { flower: 1, nectar: 1 },
        interactions: ['inspect', 'pick_flower']
      }));
    } else if (tile.type === 'stone' && r > 0.82) {
      out.push(makeObject('stone_node', tile, 1, 1, {
        hp: 50,
        height: 0.7,
        growth: 100,
        resources: { stone: 4 },
        interactions: ['inspect', 'mine_stone']
      }));
    }
  ...
  function getChunkAtTile(tx, ty, reason = 'view') {
    const c = chunkOfTile(tx, ty);
    return ensureChunk(c.cx, c.cy, reason);
  }

  function tileAt(tx, ty) {
    const chunk = getChunkAtTile(tx, ty, 'query');
  ...
  function objectsNear(tx, ty, radius) {
    ...
    for (let cy = c0.cy; cy <= c1.cy; cy += 1) {
      for (let cx = c0.cx; cx <= c1.cx; cx += 1) {
        const chunk = ensureChunk(cx, cy, 'human_scan');
  ...
  function isOccupied(tx, ty) {
    const chunk = getChunkAtTile(tx, ty, 'occupy_check');
  ...
  function spawnObjectNear(type, aroundTile, data) {
    for (let attempt = 0; attempt < 8; attempt += 1) {
      ...
      const chunk = getChunkAtTile(tx, ty, 'spawn_object');
  ...
  function updateWorld() {
    ...
    for (const chunk of chunks.values()) {
      for (const obj of chunk.objects) {
        ...
        if (obj.type === 'flower' && obj.growth < 100) obj.growth += 0.03;
        if (obj.type === 'flower' && obj.growth >= 100 && tick % 240 === 0 && hash(obj.x, obj.y, tick) > 0.72) {
          spawnObjectNear('flower', obj, { hp: 8, height: 0.25, growth: 0, resources: { flower: 1, nectar: 1 }, interactions: ['inspect', 'pick_flower'] });
        }
        if (obj.type === 'apple_tree' && tick % 180 === 0 && hash(obj.x, obj.y, tick) > 0.58) {
  ...
  function updateHuman(human) {
    human.hunger = Math.min(100, human.hunger + 0.015);
    human.energy = Math.max(0, human.energy - 0.004);

    const tx = Math.round(human.x);
    const ty = Math.round(human.y);
    ensureExploredAround(tx, ty);

    if (!human.target || dist(human.x, human.y, human.target.x, human.target.y) < 0.15) {
      chooseHumanTarget(human);
    }
    ...
    if (human.target) {
      ...
      const tile = tileAt(Math.round(human.x + dx / len * 0.4), Math.round(human.y + dy / len * 0.4));
      const speed = tile.passable ? 0.012 / tile.walkCost : 0.002;
    ...
    for (const obj of hereObjects) {
      if (obj.type === 'apple' && human.hunger > 35) {
        obj.state.depleted = true;
        human.inventory.apple += 1;
        human.hunger = Math.max(0, human.hunger - 38);
        human.thought = 'съел яблоко';
        human.memory.unshift('нашёл еду на ' + obj.x + ',' + obj.y);
        human.target = null;
        break;
      }
      if (obj.type === 'flower' && human.energy > 35 && hash(obj.x, obj.y, tick) > 0.985) {
        obj.state.depleted = true;
  ...
  function chooseHumanTarget(human) {
    const tx = Math.round(human.x);
    const ty = Math.round(human.y);
    const nearby = objectsNear(tx, ty, 7);
    ...
    if (human.hunger > 35) {
      ...
      const tree = nearby.find(o => o.type === 'apple_tree');
      if (tree) {
        human.target = { x: tree.x, y: tree.y, reason: 'wait_for_tree' };
        human.action = 'inspect_tree';
        human.thought = 'идёт к яблоне';
        return;
      }
    }

    const angle = hash(tx, ty, tick + human.id.length) * Math.PI * 2;
    const radius = 5 + Math.floor(hash(ty, tx, tick + 17) * 10);
    let nx = Math.round(tx + Math.cos(angle) * radius);
    let ny = Math.round(ty + Math.sin(angle) * radius);
    let tile = tileAt(nx, ny);
    for (let i = 0; i < 8 && !tile.passable; i += 1) {
      nx = tx + Math.floor(hash(tx, ty, tick + i) * 15) - 7;
      ny = ty + Math.floor(hash(ty, tx, tick + i + 20) * 15) - 7;
      tile = tileAt(nx, ny);
    }
    human.target = { x: nx, y: ny, reason: 'explore' };
    human.action = 'explore';
    human.thought = 'исследует новый чанк';
  }

  function ensureExploredAround(tx, ty) {
    const c = chunkOfTile(tx, ty);
    for (let cy = c.cy - 1; cy <= c.cy + 1; cy += 1) {
      for (let cx = c.cx - 1; cx <= c.cx + 1; cx += 1) ensureChunk(cx, cy, 'human_exploration');
    }
  }
  ...
  function homeCamera() {
    camera.x = 0;
    camera.y = 0;
    camera.zoom = window.innerWidth < 600 ? 1.6 : 1.9;
  }
  ...
  function followHumans() {
    ...
    camera.x += (cx - camera.x) * 0.02;
    camera.y += (cy - camera.y) * 0.02;
  }
  ...
  function render() {
    ...
    for (let cy = c0.cy; cy <= c1.cy; cy += 1) {
      for (let cx = c0.cx; cx <= c1.cx; cx += 1) drawChunkTiles(ensureChunk(cx, cy, 'viewport'));
    }
    drawChunkGrid(c0, c1);
    for (let cy = c0.cy; cy <= c1.cy; cy += 1) {
      for (let cx = c0.cx; cx <= c1.cx; cx += 1) drawChunkObjects(ensureChunk(cx, cy, 'viewport'));
    }
  ...
  function visibleTileBounds() {
    ...
    return {
      x0: Math.floor((camera.x - halfW) / TILE) - 2,
      y0: Math.floor((camera.y - halfH) / TILE) - 2,
      x1: Math.ceil((camera.x + halfW) / TILE) + 2,
      y1: Math.ceil((camera.y + halfH) / TILE) + 2
    };
  }

  function drawChunkTiles(chunk) {
    for (const tile of chunk.tiles) drawTile(tile);
  }
  ...
  function drawObject(obj) {
    ...
    ctx.beginPath();
    ctx.ellipse(x, y + 8, TILE * 0.38, 4, 0, 0, Math.PI * 2);
    ctx.fill();
    ...
    if (obj.type === 'apple_tree') {
      ...
      ctx.fillRect(x - 14, y - 20, 28, 18);
      ctx.fillStyle = '#4f873d';
      ctx.fillRect(x - 10, y - 29, 22, 18);
      ctx.fillStyle = '#d8554f';
      ctx.fillRect(x + 5, y - 17, 4, 4);
    } else if (obj.type === 'flower') {
      const grown = obj.growth >= 100;
      ctx.fillStyle = grown ? '#f5e9bf' : '#d9ad61';
      ctx.fillRect(x - 3, y - 6, 5, 5);
      ctx.fillStyle = '#326633';
      ctx.fillRect(x, y - 1, 3, 8);
    } else if (obj.type === 'apple') {
      ctx.fillStyle = '#cf4747';
      ctx.fillRect(x - 5, y - 5, 10, 10);
      ctx.fillStyle = '#8fc45c';
      ctx.fillRect(x + 1, y - 8, 4, 3);
    } else if (obj.type === 'stone_node') {
      ctx.fillStyle = '#444a47';
      ctx.fillRect(x - 9, y - 2, 18, 9);
      ctx.fillStyle = '#8a8f83';
      ctx.fillRect(x - 5, y - 8, 13, 8);
    }
  }
  ...
  function drawHuman(human) {
    ...
    ctx.fillText(human.name, x, y - 23);
  }

  function drawChunkGrid(c0, c1) {
    ctx.strokeStyle = 'rgba(255,235,180,.08)';
    ctx.lineWidth = 1 / camera.zoom;
    for (let cx = c0.cx; cx <= c1.cx + 1; cx += 1) {
      ctx.beginPath();
      ctx.moveTo(cx * CHUNK * TILE, c0.cy * CHUNK * TILE);
      ctx.lineTo(cx * CHUNK * TILE, (c1.cy + 1) * CHUNK * TILE);
      ctx.stroke();
    }
    for (let cy = c0.cy; cy <= c1.cy + 1; cy += 1) {
      ctx.beginPath();
      ctx.moveTo(c0.cx * CHUNK * TILE, cy * CHUNK * TILE);
      ctx.lineTo((c1.cx + 1) * CHUNK * TILE, cy * CHUNK * TILE);
      ctx.stroke();
    }
  }
  ...
  function zoomAt(clientX, clientY, nextZoom) {
    ...
    camera.zoom = Math.max(0.35, Math.min(4, nextZoom));
  ...
  function selectAt(clientX, clientY) {
    ...
    const chunk = getChunkAtTile(tx, ty, 'select');
    let selected = null;
    for (const human of humans) {
      if (Math.hypot(human.worldX - p.x, human.worldY - p.y) < 14) selected = human;
    }
    if (!selected) {
      for (const obj of chunk.objects) {
        if (!obj.state.depleted && Math.hypot(obj.worldX - p.x, obj.worldY - p.y) < 14) selected = obj;
      }
    }
    if (!selected) selected = tileAt(tx, ty);
  ...
  function bindInput() {
    canvas.addEventListener('wheel', e => {
      e.preventDefault();
      zoomAt(e.clientX, e.clientY, camera.zoom * (e.deltaY > 0 ? 0.88 : 1.14));
    }, { passive: false });
    ...
    canvas.addEventListener('pointerup', e => {
      camera.drag = false;
      if (camera.moved < 6) selectAt(e.clientX, e.clientY);
    });
  ...
  ensureChunk(0, 0, 'start');
  ensureExploredAround(0, 0);
  bindInput();
  ...
  function loop(now) {
    ...
    lastTime = now;
    if (!paused) {
      const steps = Math.min(4, Math.floor(dt / 16) || 1);
